use crate::{
    auth_header::{AuthHeader, AuthHeaderType},
    cookie::{from_tower_cookie_deref, Cookie},
};
...
impl Client {
    ...
    pub async fn do_get(&self, url: &str) -> Result<Response> {
        ...
        let mut prepared_req = self.reqwest_client.get(&url);
        prepared_req = self.append_auth_header_if_exists(prepared_req);

        let reqwest_res = prepared_req.send().await?;
    ...
    pub async fn do_delete(&self, url: &str) -> Result<Response> {
        ...
        let mut prepared_req = self.reqwest_client.delete(&url);
        prepared_req = self.append_auth_header_if_exists(prepared_req);

        let reqwest_res = prepared_req.send().await?;
        self.capture_response(Method::DELETE, url, reqwest_res)
            .await
    }
        ...
        T: DeserializeOwned,
    {
        self.do_get(url)
            .await
            .and_then(|res| res.json_body_as::<T>())
    }
    ...
    pub fn add_auth_header(&mut self, kind: AuthHeaderType, content: String) -> Result<()> {
        self.auth_header = Some(AuthHeader { kind, content });

        Ok(())
    }

    fn append_auth_header_if_exists(&self, request: RequestBuilder) -> RequestBuilder {
    ...
    async fn do_push(&self, method: Method, url: &str, content: PostContent) -> Result<Response> {
        ...
        if !matches!(method, Method::POST | Method::PUT | Method::PATCH) {
            return Err(Error::NotSupportedMethodForPush {
                given_method: method,
            });
        }
        let reqwest_res = match content {
            PostContent::Json(value) => {
                let mut prepared_req = self
                    .reqwest_client
